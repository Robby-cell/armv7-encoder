...
fn push_pop_range() {
    ...
    let bytes = assemble("push {r0-r5}").unwrap().bytes;
    assert_eq!(bytes, [0x3f, 0x00, 0x2d, 0xe9]);

    // pop {r1-r3, pc} -> r1, r2, r3, pc (15).
    // mask = 0b1000_0000_0000_1110 = 0x800E
    // Pop base = 0x08BD0000 -> 0xE8BD800E
    let bytes = assemble("pop {r1-r3, pc}").unwrap().bytes;
    assert_eq!(bytes, [0x0e, 0x80, 0xbd, 0xe8]);
}
...
fn ldm_stm() {
    ...
    assert_eq!(bytes, [0x0e, 0x00, 0x90, 0xe8]);

    let code = "stm r0, {r2, r4-r5}";
    let bytes = assemble(code).unwrap().bytes;
    // base=0x08800000. mask=r2,r4,r5=0x0034. -> 0xE8800034
    assert_eq!(bytes, [0x34, 0x00, 0x80, 0xe8]);
}

#[test]
fn ldm_stm_writeback() {
    let code = "ldmia r1!, {r2-r4, pc, sp}";
    let bytes = assemble(code).unwrap().bytes;
    // cond AL=0xE. base=0x08900000. U=1, W=1, L=1 -> +0x00B00000. Rn = r1 -> +0x00010000
    // register mask:
    //   r2 = 0x0004
    //   r3 = 0x0008
    //   r4 = 0x0010
...
fn ldr_immediate_offset() {
    ...
    let bytes = assemble(code).unwrap().bytes;
    assert_eq!(bytes, [0x04, 0x00, 0x91, 0xe5]);
}
...
fn str_immediate_offset() {
    ...
    let bytes = assemble(code).unwrap().bytes;
    assert_eq!(bytes, [0x08, 0x00, 0x81, 0xe5]);
}
...
fn ldrb_register_offset() {
    ...
    let bytes = assemble(code).unwrap().bytes;
    assert_eq!(bytes, [0x02, 0x00, 0xd1, 0xe7]);
}
...
fn strb_scaled_offset() {
    ...
    let bytes = assemble(code).unwrap().bytes;
    assert_eq!(bytes, [0x02, 0x01, 0xc1, 0xe7]);
}
...
fn ldr_pc_relative() {
    ...
    let bytes = assemble(code).unwrap().bytes;
    assert_eq!(bytes, [0x08, 0x00, 0x9f, 0xe5]);
}
